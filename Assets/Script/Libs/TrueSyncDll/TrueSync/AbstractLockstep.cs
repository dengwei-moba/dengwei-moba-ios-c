...
namespace TrueSync
{
	public abstract class AbstractLockstep
	{
		...
		private const byte CHECKSUM_CODE = 198; // 校验,验证码编码

		private const byte SEND_CODE = 199;// 发送编码

		private const byte SIMULATION_EVENT_PAUSE = 0;// 模拟器时间--暂停

		private const byte SIMULATION_EVENT_RUN = 1;// 模拟器时间--运行

		private const byte SIMULATION_EVENT_END = 3;// 模拟器时间--结束

		private const int MAX_PANIC_BEFORE_END_GAME = 5;// 游戏结束前 最大等待所有玩家输入的帧

		private const int SYNCED_INFO_BUFFER_WINDOW = 3;// 同步信息 缓存窗口
        */
		internal Dictionary<byte, TSPlayer> players;// 添加玩家

		internal List<TSPlayer> activePlayers;

        internal List<SyncedData> auxPlayersSyncedData;  //DV 用来临时存储当前所有玩家来自网络的输入?

		internal List<InputDataBase> auxPlayersInputData;  //DV 用来临时存储当前所有玩家的输入?
        ...
        public static AbstractLockstep NewInstance(float deltaTime, ICommunicator communicator, IPhysicsManagerBase physicsManager, int syncWindow, int panicWindow, int rollbackWindow, TrueSyncEventCallback OnGameStarted, TrueSyncEventCallback OnGamePaused, TrueSyncEventCallback OnGameUnPaused, TrueSyncEventCallback OnGameEnded, TrueSyncPlayerDisconnectionCallback OnPlayerDisconnection, TrueSyncUpdateCallback OnStepUpdate, TrueSyncInputDataProvider InputDataProvider)
		{
			...
			if (flag)
			{
				result = new DefaultLockstep(deltaTime, communicator, physicsManager, syncWindow, panicWindow, rollbackWindow, OnGameStarted, OnGamePaused, OnGameUnPaused, OnGameEnded, OnPlayerDisconnection, OnStepUpdate, InputDataProvider);
			}
			else
			{
				result = new RollbackLockstep(deltaTime, communicator, physicsManager, syncWindow, panicWindow, rollbackWindow, OnGameStarted, OnGamePaused, OnGameUnPaused, OnGameEnded, OnPlayerDisconnection, OnStepUpdate, InputDataProvider);
			}
			return result;
		}
        ...
        public void CheckGameStart()
        {
            ...
            {
                if (TrueSyncManager.Instance.isGameStarted)
                {
                    this.RunSimulation(false);
