...
namespace TrueSync
{
    ...
    public struct TSQuaternion
    {
        ...
        /// <summary>
        /// DV 创建一个从fromDirection到toDirection的旋转。
        /// </summary>
        ...
        /// <summary>
        /// DV 没有SetLookRotation调用???
        /// DV 创建一个旋转，沿着forward（z轴）并且头部沿着upwards（y轴）的约束注视。也就是建立一个旋转，使z轴朝向view  y轴朝向up。
        ...
        /// <summary>
        /// DV 球形插值，通过t值from向to之间插值。
        /// 例子:transform.rotation = Quaternion.Slerp (from.rotation, to.rotation, Time.time * speed);
        /// </summary>
        ...
        /// <summary>
        /// DV 欧拉角:返回一个旋转角度，绕z轴旋转z度，绕x轴旋转x度，绕y轴旋转y度（像这样的顺序）。
        /// 例子:绕y轴旋转30度
        ...
        /// <summary>
        /// DV 从fromDirection到toDirection创建一个旋转。
