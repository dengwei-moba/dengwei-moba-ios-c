...
namespace TrueSync {
    ...
    public partial struct FP : IEquatable<FP>, IComparable<FP> {
        ...
        public static readonly FP PositiveInfinity = new FP(MAX_VALUE);     //表示正无穷，也就是无穷大，∞
        public static readonly FP NegativeInfinity = new FP(MIN_VALUE+1);   //表示负无穷，也就是无穷小，-∞ 
        ...
        public static readonly FP Epsilon = FP.EN3;                         //一个很小的浮点数值
        ...
        public static readonly FP Deg2Rad = Pi / new FP(180);  //DV 角度转弧度

        public static readonly FP Rad2Deg = new FP(180) / Pi; //DV 弧度转角度(角度=弧度*Rad2Deg)
        ...
        /// DV 如果该值位于偶数和不均匀值之间的一半，则返回偶数值。 (即如果数字末尾是.5，因此它是在两个整数中间，不管是偶数或是奇数，将返回偶数)
        /// </summary>
        ...
        /// DV 返回由参数 f 指定的角的余弦值（介于 -1.0 与 1.0 之间的值）。
        ...
        /// <summary>
        /// DV 计算并返回参数 f 中指定的数字的反正切值。返回值介于负二分之 pi 与正二分之 pi 之间。
        ...
        /// <summary>
        /// DV 反正切2:以弧度为单位计算并返回 y/x 的反正切值。返回值表示相对直角三角形对角的角，其中 x 是临边边长，而 y 是对边边长。
        ...
        /// 例子:
            ...
            /// 注:通常使用transform.lookAt.但这可以给你更多的对角度的控制
        /// </summary>
        ...
        /// <summary>
        /// DV 反正弦:以弧度为单位计算并返回参数 f 中指定的数字的反正弦值。
        /// </summary>
        ...
        /// <summary>
        /// DV 反余弦:以弧度为单位计算并返回参数 f 中指定的数字的反余弦值。
        ...
        public static FP Acos(FP x)
        {
            if (x < -One || x > One)
            {
                Debug.LogErrorFormat("Must between======>{0},{1}", x._serializedValue, One._serializedValue);
