...
namespace TrueSync {
    ...
    public class TSRigidBody : MonoBehaviour {
        ...
        /// <summary>
        /// DV 插值允许你以固定的帧率平滑物理运行效果
        ///  插值默认是关闭的。通常刚体插值用于玩家角色。物理是不连续的时间步运行，而显卡以可变的帧率渲染。这可能导致物体抖动，因为物理和显卡不完全同步。这个效果是细微的但是通常会在玩家角色上看到，尤其是如果相机跟随主角色。建议为主角色打开插值，但其他的禁用
        /// </summary>
        ...
        public void AddForceAtPosition(TSVector force, TSVector position) {
            ...
            //AddForceAtPosition(force, position, ForceMode.Impulse);//DV新版本是这个参数
        }
